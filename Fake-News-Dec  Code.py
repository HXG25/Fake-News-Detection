...
import nltk
import re
from wordcloud import WordCloud  # Make sure to install this module
...
fake = pd.read_csv("D:/NLP prototype/Prototype 2 M/Fake FND.csv")
fake.head()
...
fig = plt.figure(
    ...
    facecolor = 'k',
    edgecolor = 'k')
plt.imshow(wordcloud, interpolation = 'bilinear') #this line draw the photo 
plt.axis('off')
plt.tight_layout(pad=0)
plt.show()
del text
'''Concatenating Text:
The loop for news in fake.text.values: iterates over each element in the
"text" column of your DataFrame (fake). For each news article, it appends its content to the text variable.
WordCloud Generation:

1- The WordCloud object is created with specific parameters:

width and height: Dimensions of the word cloud image.

background_color: The background color of the word cloud (black in this case).

2-stopwords: A set of English stopwords from NLTK. Stopwords are common words like "the," "and," etc.

     , that are often removed in text analysis to focus on meaningful words.
     The generate(text) method is called to process the concatenated text and generate the word cloud.

3-Plotting the Word Cloud:

  1-fig = plt.figure(...) creates a figure object with specified dimensions and colors.

  2-plt.imshow(wordcloud, interpolation='bilinear') displays the word cloud on the figure.

  3-plt.axis('off') removes the axis labels.

  4-plt.tight_layout(pad=0) adjusts the layout to remove unnecessary whitespace.

  5-plt.show() displays the word cloud.

4-Memory Management:

del text deletes the text variable to free up memory. The word cloud is already generated,
and this step is to manage memory usage.
'''

real = pd.read_csv("D:/NLP prototype/Prototype 2 M/True FND.csv")
real.head()
...
w2v_model.wv.get_vector("corona")
...
# Takes RAM 
word_vectors = KeyedVectors.load_word2vec_format('F:/GoogleNews-vectors-negative300.bin', binary=True)
EMBEDDING_DIM=300
